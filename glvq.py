...
class GlvqModel(_LvqBaseModel):
    ...
    def _optgrad(self, variables, training_data, label_equals_prototype,
        ...
        mu = self.beta * np.exp(self.beta * mu) / (
            1 + np.exp(self.beta * mu)) ** 2
        ...
        g = g * (1 + 0.001 * random_state.rand(*g.shape) - 0.5)
        return g.ravel()
    ...
    def _optfun(self, variables, training_data, label_equals_prototype):
        ...
        distcorrectpluswrong = ne.evaluate("distcorrect + distwrong")
        distcorectminuswrong = ne.evaluate("distcorrect - distwrong")
        mu = ne.evaluate("distcorectminuswrong / distcorrectpluswrong")
