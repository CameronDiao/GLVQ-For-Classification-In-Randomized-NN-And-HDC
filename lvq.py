...
class _LvqBaseModel(BaseEstimator, ClassifierMixin):
    ...
    def _validate_train_parms(self, train_set, train_lab):
        ...
        # initialize prototype coefficient vectors
        if self.initial_prototypes is None:
            self.w_ = np.empty([np.sum(nb_ppc), nb_samples], dtype=np.double)
            self.c_w_ = np.empty([nb_ppc.sum()], dtype=self.classes_.dtype)
            pos = 0
            for actClass in range(nb_classes):
                nb_prot = nb_ppc[actClass]
                gamma = np.zeros((nb_prot, nb_samples))
                mask = train_lab == self.classes_[actClass]
                #gamma[:, mask] = random_state.dirichlet(np.ones(np.count_nonzero(mask)), size = 1) # elements drawn from a Dirichlet distribution
                gamma[:, mask] = 1 / mask.size
                self.w_[pos:pos + nb_prot] = gamma
