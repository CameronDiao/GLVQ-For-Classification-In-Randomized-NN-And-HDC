...
class PyTorchObjective(object):
    ...
    def __init__(self, obj_module, obj_loss, train_data, train_labels):
    ...
    def cache(self, x):
        ...
        obj = self.f(self.train_data)
        loss = self.loss(obj, self.train_labels)
        # backprop the objective
        loss.backward()
        self.cached_f = loss.item()
        self.cached_jac = self.pack_grads().astype('float64')
